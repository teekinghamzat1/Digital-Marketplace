...
export function Footer({ settings: propSettings }: FooterProps) {
  ...
  const siteName = settings?.siteName || "Sumon Mondal Logs Marketplace";
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
        ...
        <div className="col-span-1 md:col-span-2">
          ...
          <div className="flex flex-wrap items-center gap-4">
            {settings?.whatsapp && (
              <a href={`https://wa.me/${settings.whatsapp.replace(/\+/g, '')}`} target="_blank" className="w-10 h-10 rounded-xl bg-surface-elevated flex items-center justify-center text-text-secondary hover:text-primary transition-all border border-border-default hover:shadow-lg hover:shadow-primary/5">
            ...
            {settings?.email && (
              <a href={`mailto:${settings.email}`} className="w-10 h-10 rounded-xl bg-surface-elevated flex items-center justify-center text-text-secondary hover:text-primary transition-all border border-border-default hover:shadow-lg hover:shadow-primary/5">
                <Mail size={18} />
              </a>
            )}
          </div>
        ...
        {/* Quick Links */}
        <div>
          <h4 className="font-bold text-foreground mb-6 font-syne uppercase tracking-wider text-xs">Marketplace</h4>
          <ul className="space-y-4">
            <li>
              <Link href="/shop" className="text-text-secondary hover:text-primary transition-colors text-sm font-medium">
                Browse Shop
              </Link>
            </li>
            <li>
        ...
        {/* Legal */}
        <div>
          <h4 className="font-bold text-foreground mb-6 font-syne uppercase tracking-wider text-xs">Legal</h4>
          <ul className="space-y-4">
            <li>
              <Link href="/privacy-policy" className="text-text-secondary hover:text-primary transition-colors flex items-center gap-2 text-sm font-medium">
                <Shield size={16} />
                Privacy Policy
              </Link>
            </li>
            <li>
              <Link href="/terms-and-conditions" className="text-text-secondary hover:text-primary transition-colors flex items-center gap-2 text-sm font-medium">
                <FileText size={16} />
                Terms & Conditions
              </Link>
            </li>
